...
$id  = (int)($_GET['id'] ?? 0);
$ref = (new Reference())->findById($id);
if (!$ref) { setFlash('error', 'Reference not found.'); redirect('views/admin/references/index.php'); }
...
<div class="container px-4" style="max-width:650px">
    ...
    <div class="d-flex align-items-center mb-4">
        ...
        <h1 class="h3 fw-bold mb-0">Edit Reference</h1>
    </div>
            ...
            <form method="POST" action="<?= BASE_URL ?>controllers/process.php" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control" required
                           value="<?= sanitize($ref['title']) ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Author(s) <span class="text-danger">*</span></label>
                    <input type="text" name="author" class="form-control" required
                           value="<?= sanitize($ref['author']) ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-semibold">Year <span class="text-danger">*</span></label>
                    <input type="number" name="year" class="form-control" required
                           min="1900" max="<?= date('Y') ?>" value="<?= $ref['year'] ?>">
                </div>
                <div class="mb-4">
                    <label class="form-label fw-semibold">Full Citation <span class="text-danger">*</span></label>
                    <textarea name="citation" class="form-control" rows="3" required><?= sanitize($ref['citation']) ?></textarea>
                </div>

                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-warning px-4">
                        <i class="bi bi-save"></i> Update Reference
                    </button>
